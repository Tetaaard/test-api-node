const db = require("./db");
const { createUser, updateUserField } = require("../controller/user");
...
describe("test user controller", () => {
  ...
  it("update user's mail with success", async () => {
    const user = await createUser("jeremy", "dubar.jeremy@gmail.com");
    const userUpdated = await updateUserField("email", user.email, "newEmail@gmail.com");
  ...
  it("update user's mail with error (currentEmail that does not exists)", async () => {
    await expect(updateUserField("email","email@gmail.com", "newEmail@gmail.com")).rejects.toThrow();
  });
  ...
  it("update user's mail with error (mail already exist in the Db)", async () => {
    ...
    await expect(updateUserField("email",jeremy.email, garance.email)).rejects.toThrow();
  });
});
